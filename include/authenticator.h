#pragma once


#include <Arduino.h>
#include "crypto.h"
#include "utils.h"

class Authenticator
{
public:
    const uint8_t certificate[789] = {
        };

    const size_t certificateSize = sizeof(certificate);

    uint8_t* getCertificate()
    {
        return (uint8_t*)&certificate[0];
    }

    size_t getCertificateSize ()
    {
        return certificateSize;
    }

    void sign(const AuthenticatorData *authenticatorData, const uint8_t *clientDataHash, uint8_t *signature, size_t *signatureSize)
        {
            const size_t AuthDataWithAttSize = sizeof(AuthenticatorData);
            const size_t AuthDataNoAttSize = sizeof(AuthenticatorData) - sizeof(AttestedCredentialData);

            // construct data for signing
            uint8_t buffer[AuthDataWithAttSize + 32];
            size_t bufferSize;
            if (authenticatorData->flags.f.attestationData)
            {
                memcpy(buffer, authenticatorData, AuthDataWithAttSize);
                memcpy(buffer + AuthDataWithAttSize, clientDataHash, 32);

                bufferSize = AuthDataWithAttSize + 32;
            }
            else
            {
                memcpy(buffer, authenticatorData, AuthDataNoAttSize);
                memcpy(buffer + AuthDataNoAttSize, clientDataHash, 32);

                bufferSize = AuthDataNoAttSize + 32;
            }

            //
            uint8_t hash[32];
            Crypto c;
            c.SHA256_hash(buffer, bufferSize, hash);

            Serial.println("Hash:");            
            utils::serialDumpBuffer(hash, 32, "Hash");

            //
            ECDSA ecdsa;
            uint8_t signatureBuf[64];
            ecdsa.sign(hash, signatureBuf);

            Serial.println("Signature:");
            utils::serialDumpBuffer(signatureBuf, 64, "Sign");

            //
            ecdsa.encodeSignature(signatureBuf, signature, signatureSize);

            Serial.println("Encoded signature:");
            utils::serialDumpBuffer(signature, *signatureSize);
            Serial.printf("%d\n", *signatureSize);
        }
};